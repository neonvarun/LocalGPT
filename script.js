document.addEventListener('DOMContentLoaded', function () {
    ...
    function appendMessage(sender, message) {
        ...
        avatar.style.backgroundImage = `url('${sender === 'user' ? 'user.png' : 'localgpt.png'}')`;

        const messageContent = document.createElement('div');
        messageContent.className = 'message-content';

        if (sender === 'localgpt') {
            const formattedMessage = formatLocalGPTMessage(message);
    ...
    function formatLocalGPTMessage(message) {
    ...
    function showLoadingIndicator() {
        ...
        loadingDiv.className = 'message localgpt-message loading';
        loadingDiv.innerHTML = '<div class="avatar" style="background-image: url(\'localgpt.png\');"></div><div class="message-content"><div class="typing-indicator"><span></span><span></span><span></span></div></div>';
    ...
    function fetchResponse(message) {
        ...
        fetch('http://localhost:5001/ask', {
            ...
            body: JSON.stringify({ input: message, sessionId: currentChatIndex.toString() }),
        })
            .then(response => response.json())
            .then(data => {
                chatContainer.removeChild(loadingIndicator);
                if (data.response) {
                    appendMessage('localgpt', data.response);
                    chatHistory[currentChatIndex].messages.push({ sender: 'localgpt', content: data.response });
                } else {
                    appendMessage('localgpt', 'Sorry, I encountered an error.');
                    chatHistory[currentChatIndex].messages.push({ sender: 'localgpt', content: 'Sorry, I encountered an error.' });
                }
            })
            .catch(error => {
    ...
    function formatChatHistory() {
        ...
        chatHistory[currentChatIndex].messages.forEach(msg => {
            formattedChat += `${msg.sender === 'user' ? 'User' : 'LocalGPT'}: ${msg.content}\n\n`;
        });
    ...
    function clearCurrentChat() {
        ...
        if (currentChatIndex !== -1) {
            ...
            fetch('http://localhost:5001/clear-chat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ sessionId: currentChatIndex.toString() }),
            })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        console.log('Chat session cleared on server');
                    } else {
                        console.error('Failed to clear chat session on server');
                    }
                })
                .catch(error => {
